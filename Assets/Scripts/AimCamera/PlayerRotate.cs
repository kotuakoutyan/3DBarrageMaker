...
namespace Camerawork
{
    public class PlayerRotate :BaseCamerawork
    {
        ...
        override protected void Start()
        {
            ...
            GameObject.FindGameObjectWithTag("Player").GetComponent<FlyCharacterCore>().LocalVelocity.DistinctUntilChanged().Where(_ => IsOnGround).Subscribe(x => RotatePlayerOnGround(x));
            this.FixedUpdateAsObservable().Where(_ => !IsOnGround).Subscribe(_ => RotatePlayerInAir());
        }
