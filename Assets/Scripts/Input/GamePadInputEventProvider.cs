...
namespace Inputs
{
    public class GamePadInputEventProvider : MonoBehaviour, IInputEventProvider
    {
        ...
        {
            this.UpdateAsObservable().Select(_ => Input.GetButton("Fire2")).DistinctUntilChanged().Subscribe(x => a_Button.Value = x);
            this.UpdateAsObservable().Select(_ => Input.GetButton("Fire1")).DistinctUntilChanged().Subscribe(x => b_Button.Value = x);
            this.UpdateAsObservable().Select(_ => Input.GetButton("Fire3")).DistinctUntilChanged().Subscribe(x => x_Button.Value = x);
            this.UpdateAsObservable().Select(_ => Input.GetButton("Fire4")).DistinctUntilChanged().Subscribe(x => y_Button.Value = x);
